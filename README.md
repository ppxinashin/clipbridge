...
当前 macOS 版本尚未进行 Apple Developer ID 签名和公证。macOS 可能将从浏览器下载的应用提示为“已损坏”，这通常是 Gatekeeper 的安全拦截，并不代表安装包内容真的损坏。
...
该命令只移除 ClipBridge 的下载隔离属性，不会关闭 macOS 的全局安全保护。请勿使用 `spctl --master-disable`。
