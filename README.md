...
请从 [GitHub Releases](https://github.com/ppxinashin/clipbridge/releases) 下载对应平台的安装包：

- Apple 芯片 Mac：下载 `ClipBridge_*_aarch64.dmg`
- Windows x64：下载 `.msi` 或 `.exe` 安装包

#### macOS 首次打开提示

macOS 版本使用免费的 Ad-hoc 签名，但尚未进行 Apple Developer ID 签名和公证，因此首次打开时系统仍可能提示无法验证开发者。

1. 确认安装包来自本项目的 GitHub Releases。
2. 打开 DMG，将 `ClipBridge.app` 拖入“应用程序”文件夹。
3. 尝试打开一次，然后进入“系统设置 → 隐私与安全性”，找到 ClipBridge 提示并点击“仍要打开”。
...
Ad-hoc 签名可以保证应用包在下载后保持完整，但无法替代 Apple 公证。上述命令只移除 ClipBridge 的下载隔离属性，不会关闭 macOS 的全局安全保护。请勿使用 `spctl --master-disable`。
